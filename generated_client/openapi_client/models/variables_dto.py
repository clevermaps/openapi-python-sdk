...
class VariablesDTO(BaseModel):
    """
    VariablesDTO
    """
    type: constr(strict=True) = Field(...)
    title: constr(strict=True, max_length=255, min_length=1) = Field(...)
    variables: conlist(VariableDTO, min_items=1) = Field(...)
    __properties = ["type", "title", "variables"]

    @validator('type')
    def type_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^variables$", value):
            raise ValueError(r"must validate the regular expression /^variables$/")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> VariablesDTO:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> VariablesDTO:
        """Create an instance of VariablesDTO from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return VariablesDTO.parse_obj(obj)

        _obj = VariablesDTO.parse_obj({
            "type": obj.get("type"),
            "title": obj.get("title"),
